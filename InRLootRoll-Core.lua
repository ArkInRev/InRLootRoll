...
20170126 - 1.19
	Resolved "script ran too long" error
		note:  Reduced calls to INRLR_GetRelicType() in IsAnUpgradeForCharacter()
...
20160914 - 1.10
	...
	Detailed
		Localization in:
			...
			RollReceivedEvent (use INRLR_RANDOM_ROLL_RESULT_PATTERN)
			util - INRLR_GetRealILVL (use INRLR_ITEM_LEVEL_PATTERN)
			util - INRLR_IsBoundToPlayer (use binding constants from _G)
			util - INRLR_GetRelicType (use INRLR_RELIC_TOOLTIP_TYPE_PATTERN)
...
20160912 - 1.7
	...
	Detailed
		...
		Added nil checking to INRLR_GetFullName
...
20160830 - 1.1
	...
	Detailed
		...
		In UpdateGroupInfoCache(unit), cache the character's Spec, and in INRLR_GetItemCountFromCache subtract 2 since we added Spec
  ...
  --> PerformNotify()			if loot is a tradeable and is an upgrade for someone in group, notifies based on users' preferred Notify Mode
    ...
    --> WhisperReceivedEvent()  triggered when we receive a 'TRADE' whisper from a person who looted a tradeable item
      --> AskForRolls()			prompts the group to roll for the item
        --> INRLR_EndRolls()			notifies the group who won the roll
...
TODO Refactoring
	...
	refactor - since INRLR_GetUnitNameWithRealm() operates on units, can we replace all of its calls with UnitFullName(unit)?
...
-- slash command
SLASH_InRLootRollCommand1 = '/inrlr'
...
local INRLR_RELICSLOT = 1000  -- for indexing relics in groupInfoCache
...
local currentRolls = {}
local INRLR_RANDOM_ROLL_RESULT_PATTERN = _G.RANDOM_ROLL_RESULT
	  INRLR_RANDOM_ROLL_RESULT_PATTERN = INRLR_RANDOM_ROLL_RESULT_PATTERN:gsub('%%s', '(.+)')
	  INRLR_RANDOM_ROLL_RESULT_PATTERN = INRLR_RANDOM_ROLL_RESULT_PATTERN:gsub('%%d %(%%d%-%%d%)', '(%%d+) %%((%%d+)%%-(%%d+)%%)')
	  INRLR_RANDOM_ROLL_RESULT_PATTERN = '^' .. INRLR_RANDOM_ROLL_RESULT_PATTERN .. '$'
...
local function GetFullItemInfo(item)
	...
	if item ~= nil then
		...
		if fullItemInfo[FII_IS_EQUIPPABLE] or fullItemInfo[FII_IS_RELIC] then
			...
			if t ~= nil then
				realILVL = t:match(INRLR_ITEM_LEVEL_PATTERN)
			end
			...
			while not classes and tooltip.leftside[index] do
				t = tooltip.leftside[index]:GetText()
				if t ~= nil then
					classes = t:match(INRLR_CLASSES_ALLOWED_PATTERN)
				end
				index = index + 1
			end
			if classes ~= nil then
...
function INRLR_GetRelicCountFromCache(name)
	local relicCount = 0
	local characterDetails = groupInfoCache[name]
	if characterDetails ~= nil then
		if characterDetails[INRLR_RELICSLOT + 1] ~= nil then
			relicCount = relicCount + 1
		end
		if characterDetails[INRLR_RELICSLOT + 2] ~= nil then
			relicCount = relicCount + 1
		end
		if characterDetails[INRLR_RELICSLOT + 3] ~= nil then
			relicCount = relicCount + 1
		end
	end
	return relicCount
...
function INRLR_GetItemCountFromCache(name)
	local itemCount = 0
...
local function GetEquippedRelic(characterName, relicNumber)
	...
	if characterName == INRLR_GetUnitNameWithRealm('player') then
		...
		local characterDetails = groupInfoCache[characterName]
		if characterDetails ~= nil then
			relic = characterDetails[INRLR_RELICSLOT + relicNumber]
		end
	end
	return relic
end
...
local function IsTrinketUsable(item, role)
	...
	if role == 'AgilityDPS' then
		trinketList = INRLR_TRINKET_AGILITY_DPS
	elseif role == 'IntellectDPS' then
		trinketList = INRLR_TRINKET_INTELLECT_DPS
	elseif role == 'StrengthDPS' then
		trinketList = INRLR_TRINKET_STRENGTH_DPS
	elseif role == 'Healer' then
		trinketList = INRLR_TRINKET_HEALER
...
local function IsAnUpgrade(itemILVL, equippedItem)
	local equippedILVL = INRLR_GetRealILVL(equippedItem)
...
local function IsAnUpgradeForCharacter(fullItemInfo, characterName)
	...
	if itemEquipLoc ~= nil and itemEquipLoc ~= '' then
		if itemEquipLoc == 'INVTYPE_FINGER' then
			...
			equippedILVL = min(INRLR_GetRealILVL(equippedItem0), INRLR_GetRealILVL(equippedItem1))
	...
	elseif fullItemInfo[FII_IS_RELIC] then
		...
		local relic1ILVL = INRLR_GetRealILVL(relic1)
		local relic1Type = INRLR_GetRelicType(relic1)
		local relic2 = GetEquippedRelic(characterName, 2)
		local relic2ILVL = INRLR_GetRealILVL(relic2)
		local relic2Type = INRLR_GetRelicType(relic2)
		local relic3 = GetEquippedRelic(characterName, 3)
		local relic3ILVL = INRLR_GetRealILVL(relic3)
		local relic3Type = INRLR_GetRelicType(relic3)
...
local function IsAnUpgradeForAnyCharacter(fullItemInfo)
	...
	while GetRaidRosterInfo(index) ~= nil do
		...
		if IsEquippableItemForCharacter(fullItemInfo, characterName, INRLR_CURRENT_SPEC_ONLY) then
			isAnUpgrade, equippedILVL = IsAnUpgradeForCharacter(fullItemInfo, characterName)
			if isAnUpgrade then
--				INRLR_SendDebugMessage(item .. ' is an ilvl upgrade for ' .. characterName)
				isAnUpgradeForAnyCharacterNames[#isAnUpgradeForAnyCharacterNames + 1] = INRLR_GetNameWithoutRealm(characterName) .. ' (' .. equippedILVL .. ')'
			end
		end
		index = index + 1
	end
...
local function PlayerCanCoordinateRolls()
	return (UnitIsGroupLeader('player') or UnitIsGroupAssistant('player')) and not INRLR_IsInLFR()
end
...
function INRLR_ResizeHighlights()
	for characterName, texture in pairs(raidFrameTextures) do
	...
	for characterName, tooltip in pairs(raidFrameTooltips) do
		raidFrameTooltips[characterName]:SetWidth(INRLR_HIGHLIGHT_SIZE)
		raidFrameTooltips[characterName]:SetHeight(INRLR_HIGHLIGHT_SIZE)
	end
end

function INRLR_ApplyFrameTexture(frame, characterName, item)
	local unitName = INRLR_GetUnitNameWithRealm(frame.unit)
...
local function HighlightRaidFrames(characterName, item)
	if INRLR_HIGHLIGHT_RAID_FRAMES then
		CompactRaidFrameContainer_ApplyToFrames(CompactRaidFrameContainer, "normal", INRLR_ApplyFrameTexture, characterName, item)
...
local function PerformNotify(fullItemInfo, characterName)
	...
	if INRLR_COORDINATE_ROLLS and PlayerCanCoordinateRolls() then
...
--					INRLR_SendWhisper('You can trade ' .. item .. ', which is an ilvl upgrade for ' .. names .. '. Reply \'' .. TRADE_MESSAGE .. '\' to initiate rolls for this item.', characterName)
--		end
	end
	if equippedILVL > 0 and not isAnUpgradeForLooter then
		...
		if isAnUpgradeForAnyCharacter then
			...
			if INRLR_NOTIFY_GROUP then
				if not INRLR_IsInLFR() then
					INRLR_SendBroadcast(INRLR_GetNameWithoutRealm(characterName) .. ' can trade ' .. item .. ', which is an ilvl upgrade for ' .. names)
				end
			end

			if characterName == INRLR_GetUnitNameWithRealm('player') then  -- player can trade an item
				INRLR_SendAlert('You can trade ' .. item .. ', which is an ilvl upgrade for ' .. names)
			...
			elseif IsPlayerInUpgradeList(isAnUpgradeForAnyCharacterNames) then  -- player can receive an item
				INRLR_SendAlert(INRLR_GetNameWithoutRealm(characterName) .. ' can trade ' .. item .. ', which is an ilvl upgrade for you!')
...
local function ShouldBeEvaluated(fullItemInfo)
	if not fullItemInfo[FII_IS_EQUIPPABLE] and not fullItemInfo[FII_IS_RELIC] then
		...
		if fullItemInfo[FII_REAL_ILVL] < INRLR_MIN_ILVL then
			return false
		else
			if fullItemInfo[FII_QUALITY] < INRLR_MIN_QUALITY then
				return false
			else
				if fullItemInfo[FII_BIND_TYPE] ~= LE_ITEM_BIND_ON_ACQUIRE and not (fullItemInfo[FII_BIND_TYPE] == LE_ITEM_BIND_ON_EQUIP and INRLR_INCLUDE_BOE) then
					return false
				else
					return true
				end
...
--			if currentRollItem == nil then
				-- that means the user forced rolls to end, so we can stop the countdown; relying on INRLR_EndRolls() to clean us up
--			elseif delay >= 15 and not FifteenSecondWarningDisplayed then
			if delay >= 15 and not FifteenSecondWarningDisplayed then
				INRLR_SendBroadcast('15 seconds remaining to roll for ' .. currentRollItem, false)
				FifteenSecondWarningDisplayed = true
			elseif delay >= 25 and not FiveSecondWarningDisplayed then
				INRLR_SendBroadcast('5 seconds remaining to roll for ' .. currentRollItem, false)
...
function INRLR_EndRolls()
	...
	if winners ~= nil then
		if #winners > 1 then
			INRLR_SendBroadcast(GetNames(winners) .. ' tied for ' .. currentRollItem .. ' from ' .. INRLR_GetNameWithoutRealm(currentRollOwner) .. ' with a ' .. highRoll, true)
		else
			INRLR_SendBroadcast(winners[1] .. ' won ' .. currentRollItem .. ' from ' .. INRLR_GetNameWithoutRealm(currentRollOwner) .. ' with a ' .. highRoll, true)
		end
	else
		INRLR_SendBroadcast('Nobody rolled for ' .. currentRollItem .. ' from ' .. INRLR_GetNameWithoutRealm(currentRollOwner), true)
	end
...
local function ProcessWhisper(message, sender)
	if INRLR_COORDINATE_ROLLS and PlayerCanCoordinateRolls() then
		if not string.find(sender, '-') then
			sender = INRLR_GetUnitNameWithRealm(sender)
		end
		...
		if whisperedItem ~= nil or message == TRADE_MESSAGE or message == '\'' .. TRADE_MESSAGE .. '\'' then
			...
			elseif currentRollOwner == sender then
				INRLR_SendWhisper('Your ' .. currentRollItem .. ' is currently being rolled for', sender)
			else
				local item = GetItemFromQueueByPlayer(sender)
				if item then
					INRLR_SendWhisper('Your ' .. item .. ' is already in queue to be rolled for', sender)
				else
					INRLR_SendWhisper('No record of which item you looted! Whisper \'trade [item]\' to ' .. UnitName('player') .. ' if you would still like to trade the item.', sender)
				end
			end
		end
	end
...
--			local name, roll, minRoll, maxRoll = message:match('^(.+) rolls (%d+) %((%d+)%-(%d+)%)$')
			local name, roll, minRoll, maxRoll = message:match(INRLR_RANDOM_ROLL_RESULT_PATTERN)
			if name then
				local fullname = INRLR_GetUnitNameWithRealm(name)
				if minRoll ~= '1' or maxRoll ~= '100' then
					INRLR_SendBroadcast(name .. ' rolled ' .. minRoll .. ' - ' .. maxRoll .. '; roll ignored', false)
...
local function UpdateGroupInfoCache(unit)
	local name = INRLR_GetUnitNameWithRealm(unit)

	if name ~= nil then
--		INRLR_SendDebugMessage('   Updating GroupInfoCache for ' .. name .. ', inspectIndex = ' .. inspectIndex)
...
--					INRLR_SendDebugMessage('      adding/updating item ' .. item)			
					...
					if i == _G.INVSLOT_MAINHAND or i == _G.INVSLOT_OFFHAND then
						...
						if relic1 ~= nil then
							characterDetails[INRLR_RELICSLOT + 1] = relic1
						end
						local _, relic2 = GetItemGem(item, 2)
						if relic2 ~= nil then
							characterDetails[INRLR_RELICSLOT + 2] = relic2
						end
						local _, relic3 = GetItemGem(item, 3)
						if relic3 ~= nil then
							characterDetails[INRLR_RELICSLOT + 3] = relic3
						end
					end
				end
			end
		...
		INRLR_SendDebugMessage('   Updated ' .. updatedItemCount .. ' items for ' .. name .. '; loop ' .. inspectLoop .. '; char ' .. (inspectIndex - 1))
	end
end
...
local function IsCharacterInGroup(characterFullname)
	...
	while name ~= nil do
		...
		if INRLR_GetFullName(name, GetRealmName()) == characterFullname then
			return true
		end
		index = index + 1
...
--			if CheckInteractDistance(characterName, 1) then  -- this API call has not been changed by Blizz to match the new larger inspect radius
			if UnitIsVisible(characterName) then
				...
				INRLR_wait2(DELAY_BETWEEN_INSPECTIONS, INRLR_InspectNextGroupMember)
				return true
			else
				INRLR_SendDebugMessage('   ' .. characterName .. ' out of range for inspect')
			end
		else
			INRLR_SendDebugMessage('   Unable to inspect ' .. characterName)
		end
	end
	return false
end
...
function INRLR_InspectNextGroupMember()
...
--			INRLR_SendDebugMessage('Retrying inspection for ' .. notifyInspectName)
		...
		if characterName ~= nil then  -- safeguard; character may have left the roster between the time we started the call and now
			...
			if fullname ~= nil then
				...
				if groupInfoCache[fullname] ~= nil then
					numCachedItems = INRLR_GetItemCountFromCache(fullname)
					numCachedRelics = INRLR_GetRelicCountFromCache(fullname)
...
--					INRLR_SendDebugMessage('Discontinuing inspections for ' .. fullname .. ' due to max inspect limit')
--				end
	...
	if inspectIndex > maxInspectIndex then -- that means we just completed our current loop
		if inspectLoop < MAX_INSPECT_LOOPS then
			...
			inspectIndex = 1  -- we're triggering the call to the 1st member via INRLR_InspectNextGroupMember below; increment
				-- the inspectIndex counter so the next element that gets picked up when we come back into INRLR_InspectNextGroupMember
				-- is the 2nd
			inspectLoop = inspectLoop + 1
			if queuedAnInspection then
				-- if we just queued someone for inspection, we don't need to do anything else to start the new loop since InspectGroupMember will call INRLR_InspectNextGroupMember()
			else  -- otherwise start the new loop immediately
				INRLR_InspectNextGroupMember()
			end
...
local function InspectReadyEvent(self, event, ...)
	...
	if notifyInspectName ~= nil and (notifyInspectName == name or notifyInspectName == INRLR_GetFullName(name, realm)) then  -- confirm this InspectReadyEvent is for the person we triggered for inspection.
	...
	if now - priorCacheRefreshTime > 3 then  -- only refresh if prior refresh was more than 3 seconds ago
		...
		for name, details in pairs (groupInfoCache) do
			if not IsCharacterInGroup(name) then
				INRLR_SendDebugMessage('Removing entry for ' .. name .. ' from cache')
...
--				INRLR_SendDebugMessage('Refreshing cache')
				inspectIndex = 1
				inspectLoop = 1
	...
	elseif msg == 'endroll' then
		if currentRollItem ~= nil then
			if nextRollDelay > 0 then
				INRLR_SendUserMessage('Please wait until the next roll begins')
			else
--				INRLR_SendBroadcast(UnitName('player') .. ' ended rolls', false)
				INRLR_EndRolls()
			end
		else
...
local function Initialize(self, event, addonName, ...)
	...
	if addonName == 'InRLootRoll' then
		...
		if INRLR_MIN_QUALITY == nil then
			...
			INRLR_MIN_ILVL = DEFAULT_MIN_ILVL
			INRLR_MIN_QUALITY = DEFAULT_MIN_QUALITY
		...
		if INRLR_CHECK_CHARACTER_LEVEL == nil then
			...
			INRLR_COORDINATE_ROLLS = (INRLR_NOTIFY_MODE == NOTIFY_MODE_COORDINATE_ROLLS)
			INRLR_NOTIFY_GROUP = (INRLR_NOTIFY_MODE == NOTIFY_MODE_GROUP or INRLR_NOTIFY_MODE == NOTIFY_MODE_COORDINATE_ROLLS)
...
function INRLR_PrintCache(showDetails, characterName)
	if INRLR_DEBUG then
				...
				if name == nil or name == characterName then
					...
					if details == nil then	
						...
						for slotID, item in pairs(details) do
							INRLR_SendDebugMessage('   ' .. slotID .. ' = ' .. item)
						end
					end
				end
			end
...
function INRLR_EnableDebug()
	INRLR_DEBUG = true
end

function INRLR_DisableDebug()
	INRLR_DEBUG = false
end

function INRLR_RefreshCache()
	groupInfoCache = {}
	PopulateGroupInfoCache()
end

--[[
function INRLR_TestItems(characterIndex)
	if characterIndex == nil then
		INRLR_SendDebugMessage('Usage: INRLR_TestItems(characterIndex)')
	else
		...
		for relicIndex = 1, 3 do
			if characterName == INRLR_GetUnitNameWithRealm('player') then
				...
				relic = groupInfoCache[characterName][INRLR_RELICSLOT + relicIndex]
			end
...
function INRLR_TestHighlight(item)
