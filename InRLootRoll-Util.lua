local INRLR_LONG_ADDON_NAME_PREFIX = '<' .. INRLR_LONG_ADDON_NAME .. '> '
local INRLR_SHORT_ADDON_NAME_PREFIX = '<' .. INRLR_SHORT_ADDON_NAME .. '> '
INRLR_ITEM_LEVEL_PATTERN = _G.ITEM_LEVEL
INRLR_ITEM_LEVEL_PATTERN = INRLR_ITEM_LEVEL_PATTERN:gsub('%%d', '(%%d+)')  -- 'Item Level (%d+)'
INRLR_RELIC_TOOLTIP_TYPE_PATTERN = _G.RELIC_TOOLTIP_TYPE
INRLR_RELIC_TOOLTIP_TYPE_PATTERN = INRLR_RELIC_TOOLTIP_TYPE_PATTERN:gsub('%%s', '(.+)')  -- '(.+) Artifact Relic'
	  
local tooltip

local waitTable = {};     -- used by INRLR_wait
local waitFrame = nil;    -- used by INRLR_wait

local waitTable2 = {};     -- used by INRLR_wait
local waitFrame2 = nil;    -- used by INRLR_wait

local waitTable3 = {};     -- used by INRLR_wait
local waitFrame3 = nil;    -- used by INRLR_wait
...
function INRLR_GetRelicType(item)
	local relicType = nil
...
--[[
function INRLR_IsBoundToPlayer(item)
...
function INRLR_GetRealILVL(item)
	local realILVL = nil
...
function INRLR_GetFullName(name, realm)
	if name == nil then
		return nil
...
function INRLR_GetUnitNameWithRealm(unit)
	local guid
	if unit ~= nil then
		guid = UnitGUID(unit)
	end
	if guid ~= nil then
		...
		return INRLR_GetFullName(name, realm)
	else
		return nil
	end
end

function INRLR_GetNameWithoutRealm(name)
	return (Ambiguate(name, 'short'))
end

-- given a name-realm combo, returns the GUID for that character
function INRLR_GetUnitGUIDFromFullname(fullname)
...
function INRLR_SendBroadcast(message, isHighPriority)
	SendChatMessage(INRLR_SHORT_ADDON_NAME_PREFIX .. message, GetBroadcastChannel(isHighPriority))
end	

function INRLR_SendWhisper(message, person)
	SendChatMessage(INRLR_SHORT_ADDON_NAME_PREFIX .. message, 'WHISPER', nil, person)
end

function INRLR_SendAlert(message)
	print(GetColoredMessage(INRLR_SHORT_ADDON_NAME_PREFIX, _G.YELLOW_FONT_COLOR_CODE) .. GetColoredMessage(message, _G.GREEN_FONT_COLOR_CODE))
end	

function INRLR_SendUserMessage(message)
	print(GetColoredMessage(INRLR_SHORT_ADDON_NAME_PREFIX, _G.YELLOW_FONT_COLOR_CODE) .. GetColoredMessage(message, _G.LIGHTYELLOW_FONT_COLOR_CODE))
end	

function INRLR_SendDebugMessage(message)
	if INRLR_DEBUG then
		print(GetColoredMessage(INRLR_SHORT_ADDON_NAME_PREFIX, _G.YELLOW_FONT_COLOR_CODE) .. GetColoredMessage(message, _G.GRAY_FONT_COLOR_CODE))
	end		
end	
...
function INRLR_wait(delay, func, ...)
...
function INRLR_wait2(delay, func, ...)
  if(type(delay)~='number' or type(func)~='function') then
  print('bad types')
    return false;
  end
